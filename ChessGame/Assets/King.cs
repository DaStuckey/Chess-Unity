...
public class King : MonoBehaviour
{
    ...
    public (bool, bool) CanMove(double position, GameObject piece)
    {
        ...
        if (!Checks(piece)) {
            if (Math.Abs(piece.GetComponent<Piece>().x - x) <= 1 && Math.Abs(piece.GetComponent<Piece>().y - y) <= 1)// Checks if it is a legal normal move 
            {
                ...
                {
                    Debug.Log("dsjkfklsfjskldfjlskdfjsldfjksklfjdlskfjklsfjklsfjklsfjklsjflsjflsjfslfjsljfsljflfjslfjslfjlsfjlsjfsljfslfjsldfjljflsfjls");
                    return (true, false);
                }
    ...
    public bool Checks(GameObject piece)
    {
